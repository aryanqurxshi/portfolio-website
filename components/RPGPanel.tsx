      ...
      {isOpen ? (
        ...
        >
          <motion.div
            ...
            className="relative mx-auto w-full max-w-6xl overflow-hidden rounded-[2rem] border-2 border-retro bg-retro-panel text-retro shadow-[0_0_0_1px_rgba(255,255,255,0.05),_0_30px_80px_rgba(124,58,237,0.22)]"
            onPointerDown={(event) => event.stopPropagation()}
          >
            <div className="flex items-start justify-between gap-4 border-b border-white/10 bg-black/80 px-6 py-5">
              <div>
                <p className="text-sm uppercase tracking-[0.32em] text-purple/80">RPG Menu</p>
                <h2 id="rpg-panel-title" className="mt-3 text-4xl font-semibold tracking-[0.04em] text-retro sm:text-5xl">
                  {title}
                </h2>
                {subtitle ? (
                  <p className="mt-2 max-w-2xl text-base leading-7 text-retro/70 sm:text-lg">{subtitle}</p>
                ) : null}
              </div>
              <button
                ref={closeRef}
